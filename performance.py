import scipy.special as special
import math
import time

def formula(R, V, C1, C2, t):
    power = math.exp(((R * C2 * V)-t)/(R * (C1 - C2 * V)))
    fraction = (C2 * V)/(C1 - C2 * V)
    lambert = special.lambertw(power * fraction)
    return V - (lambert * ((C1/C2)-V))


#1k ohm
R = 1e3
#5V
V = 5

C1 = 22e-6
C2 = 2.29e-6

#100ms
end = 0.1
steps = 10
dt = end/steps
t = 0

formulaStartTime = time.perf_counter()

for i in range(steps):
    t = t + dt
    result = formula(R, V, C1, C2, t).real

formulaEndTime = time.perf_counter()


sequenceStartTime = time.perf_counter()

steps = 200
dt = end/steps
t = 0
Vc = 0
for i in range(steps):
    Vr = V-Vc
    I = Vr/R
    C_bias = 22e-6 - (2.29e-6 * Vc)
    dV = (I*dt)/C_bias
    Vc = Vc + dV
    t = t + dt

sequenceEndTime = time.perf_counter()

print(f"Formula (ms): {(formulaEndTime-formulaStartTime)*10e3} ")
print(f"Sequence (ms): {(sequenceEndTime-sequenceStartTime)*10e3} ")
